...
class EvaluationApp:
    ...
    def create_info_tab(self):
        ...
        tk.Label(self.info_frame, text="Название домашней работы:").grid(row=0, column=0, sticky='w', pady=5, padx=5)
        self.hw_name_entry = tk.Entry(self.info_frame, width=30)
        self.hw_name_entry.grid(row=0, column=1, sticky='w', pady=5)

        # Количество вариантов
        tk.Label(self.info_frame, text="Количество вариантов:").grid(row=1, column=0, sticky='w', pady=5, padx=5)
        ...
        # Группа
        tk.Label(self.info_frame, text="Группа:").grid(row=2, column=0, sticky='w', pady=5, padx=5)
        self.group_var = tk.StringVar()
        self.group_combobox = ttk.Combobox(self.info_frame, textvariable=self.group_var, values=self.groups, state='readonly')
        ...
        # Студент
        tk.Label(self.info_frame, text="Студент:").grid(row=3, column=0, sticky='w', pady=5, padx=5)
        self.student_var = tk.StringVar()
        self.student_combobox = ttk.Combobox(self.info_frame, textvariable=self.student_var, values=[], state='readonly')
        ...
        # Вариант
        tk.Label(self.info_frame, text="Вариант:").grid(row=5, column=0, sticky='w', pady=5, padx=5)
        ...
        tk.Checkbutton(self.info_frame, text="Сдано вовремя", variable=self.on_time).grid(row=6, column=0, columnspan=2, sticky='w', pady=5)

    def update_student_list(self, event):
        selected_group = self.group_var.get()
        students_in_group = sorted([s for s in self.student_data if s['Группа'] == selected_group], key=lambda x: x['Фамилия'])
    ...
    def create_criteria_tab(self):
        ...
        self.criteria_inner_frame.bind("<Enter>", lambda e: canvas.bind_all("<MouseWheel>", _on_mousewheel))
        self.criteria_inner_frame.bind("<Leave>", lambda e: canvas.unbind_all("<MouseWheel>"))

        self.criteria_inner_frame.bind("<Configure>", lambda e: canvas.configure(scrollregion=canvas.bbox("all")))
    ...
    def create_section1(self):
        ...
        # 2 балла
        rb_full_match = ttk.Radiobutton(section_frame, text="2 балла — Полное соответствие изображению.", variable=var, value=2)
        rb_full_match.pack(anchor='w')
        vars_list.append((var, 2))

        # -1 балл
        rb_minor_discrepancies = ttk.Radiobutton(section_frame, text="-1 балл — Небольшие несовпадения:", variable=var, value=1)
        ...
        for text in ["Несовпадение одного из размеров.", "Небольшие отклонения в форме.", "Лишние элементы на эскизе."]:
            var_cb = tk.BooleanVar()
            cb = ttk.Checkbutton(sub_frame_minor, text=text, variable=var_cb)
            cb.pack(anchor='w')
            self.minor_discrepancies_vars.append(var_cb)
            self.minor_discrepancies_texts.append(text)

        # -2 балла
        rb_major_discrepancies = ttk.Radiobutton(section_frame, text="-2 балла — Существенные расхождения:", variable=var, value=0)
        ...
        for text in ["Отсутствие части эскиза.", "Эскиз только отдаленно напоминает вариант.", "Половина эскиза отсутствует."]:
            var_cb = tk.BooleanVar()
            cb = ttk.Checkbutton(sub_frame_major, text=text, variable=var_cb)
            cb.pack(anchor='w')
    ...
    def create_section2(self):
        ...
        # 4 балла
        rb_all_correct = ttk.Radiobutton(section_frame, text="4 балла — Все необходимые линии построения присутствуют и правильно используются.", variable=var, value=4)
        rb_all_correct.pack(anchor='w')
        vars_list.append((var, 4))

        # -1 балл за каждую ошибку
        rb_errors = ttk.Radiobutton(section_frame, text="-1 балл за каждую из следующих ошибок:", variable=var, value=-1)
        ...
        for text in [
            ...
            var_cb = tk.BooleanVar()
            cb = ttk.Checkbutton(sub_frame_errors, text=text, variable=var_cb)
            cb.pack(anchor='w')
            self.line_errors_vars.append(var_cb)
            self.line_errors_texts.append(text)

        # -4 балла
        rb_no_lines = ttk.Radiobutton(section_frame, text="-4 балла — Практически полное отсутствие линий построения.", variable=var, value=0)
    ...
    def create_section3(self):
        section_title = "3. Правильность использования линий изображения (обводки) (0–2 балла)"
        section_frame = ttk.Labelframe(self.criteria_inner_frame, text=section_title)
        section_frame.pack(fill='x', padx=10, pady=5)
        ...
        cb_correct_lines = ttk.Checkbutton(section_frame, text="2 балла — Линии обводки использованы правильно.", variable=var_correct_lines)
    ...
    def create_section4(self):
        ...
        cb_correct_dimensions = ttk.Checkbutton(section_frame, text="2 балла — Все размеры установлены корректно и соответствуют заданию.", variable=var_correct_dimensions)
    ...
    def create_penalty_tab(self):
        ...
        scrollbar = ttk.Scrollbar(self.penalty_frame, orient="vertical", command=canvas.yview)
        canvas.configure(yscrollcommand=scrollbar.set)
        ...
        self.penalty_inner_frame.bind("<Enter>", lambda e: canvas.bind_all("<MouseWheel>", _on_mousewheel))
        self.penalty_inner_frame.bind("<Leave>", lambda e: canvas.unbind_all("<MouseWheel>"))

        self.penalty_inner_frame.bind("<Configure>", lambda e: canvas.configure(scrollregion=canvas.bbox("all")))

        # Создаем штрафы
        self.create_penalties()

    def create_penalties(self):
        tk.Label(self.penalty_inner_frame, text="Дополнительные штрафы:").pack(anchor='w', pady=5)
        ...
        self.penalties = [
            ...
            ("Неиспользован необходимый массив или использован некорректно (-1 балл)", -1),
        ...
        tk.Label(delay_frame, text="Дней просрочки сдачи задания (-2 балла за каждый день):").pack(side='left')
        self.delay_entry = tk.Entry(delay_frame, width=5)
        self.delay_entry.pack(side='left', padx=5)
        self.delay_entry.insert(0, "0")

    def create_report_tab(self):
        ttk.Label(self.report_frame, text="Нажмите кнопку для формирования оценочного листа.").pack(pady=10)
        self.generate_button = ttk.Button(self.report_frame, text="Сформировать и сохранить оценочный лист", command=self.generate_report)
    ...
    def generate_report(self, save_to_file=True):
        ...
        for section, vars_list in self.criteria_scores.items():
            ...
            if section.startswith("1."):
                ...
                if score == 1:
                    ...
                    for var_cb, text in zip(self.minor_discrepancies_vars, self.minor_discrepancies_texts):
                        if var_cb.get():
                    ...
                    score = 2 - 2
                    # Добавляем комментарии из выбранных подпунктов
                    for var_cb, text in zip(self.major_discrepancies_vars, self.major_discrepancies_texts):
                        if var_cb.get():
                            comments.append(text)
                ...
                if radio_value == 4:
                    ...
                    for var_cb, text in zip(self.line_errors_vars, self.line_errors_texts):
                        if var_cb.get():
                            comments.append(text)
                elif radio_value == 0:
                    score = 0
        ...
        try:
            ...
            if delay_days > 0:
                penalty_comments.append(f"Просрочка сдачи задания на {delay_days} дней (-{2 * delay_days} балла)")
        except ValueError:
            delay_days = 0
            delay_penalty = 0
        ...
        self.create_image(section_scores, section_comments, penalty_comments, final_score, delay_days)

        # Сохранение изображения
        if save_to_file:
            self.save_image()

    def create_image(self, section_scores, section_comments, penalty_comments, final_score, delay_days):
        ...
        # Заголовок
        draw.text((img_width//2 - 150, y_position), "Оценочный лист", font=title_font, fill=text_color)
        y_position += 50
        ...
        for section, score in section_scores.items():
            ...
            y_position += 30
            draw.text((70, y_position), f"Баллы: {score}", font=text_font, fill=text_color)
            y_position += 30
            comments = section_comments.get(section, [])
            for comment in comments:
                draw.text((90, y_position), f"- {comment}", font=text_font, fill=text_color)
                y_position += 25
            y_position += 10

        # Штрафы
        draw.text((50, y_position), "Дополнительные штрафы:", font=header_font, fill=text_color)
        y_position += 30
        if penalty_comments:
            for comment in penalty_comments:
                draw.text((70, y_position), f"- {comment}", font=text_font, fill=text_color)
                y_position += 25
        else:
        ...
        # Итоговая оценка
        draw.text((50, y_position), f"Итоговая оценка: {final_score} из 10", font=header_font, fill=text_color)
    ...
    def copy_to_clipboard(self):
        ...
        if sys.platform.startswith('win'):
            ...
            self.status_var.set("Копирование изображения в буфер обмена поддерживается только на Windows.")
